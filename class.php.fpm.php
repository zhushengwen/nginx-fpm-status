	...
	public function GetPHPFPMData()
	{
		...
		$php_data = array(
			...
			'totals'            => array(
				...
				'average_duration'             => 0,
				'number_of_duration_processes' => 0,
			),
			'workers_data'      => '',
			'requests_data'     => '',
		);
		...
		array_multisort(array_column($php_fpm_data['processes'],'request duration'), SORT_DESC, $php_fpm_data['processes']);
		// Workers data
		foreach ($php_fpm_data['processes'] as $process)
		{
			if($process['request uri']=='/php-status?full&json')continue;
			if($process['request uri']=='/php-status')continue;
			$requests_row = '<tr>';
			$workers_row  = '<tr>';
			...
			foreach ($process as $name => $value)
			{
				switch ($name)
				{
					...
					case 'request duration':
						$php_data['totals']['average_duration'] += $value;
						$php_data['totals']['number_of_duration_processes'] ++;
		...
		$php_data['totals']['average_ram']      = round($php_data['totals']['average_ram']
				/ ($php_data['totals']['number_of_ram_processes'] ?: 1), 1).' '.$this->size_units[$size_unit];
		$php_data['totals']['average_cpu']      = round($php_data['totals']['average_cpu']
				/ ($php_data['totals']['number_of_cpu_processes'] ?: 1), 1).' %';
		$php_data['totals']['average_duration'] = round(($php_data['totals']['average_duration']
					/ ($php_data['totals']['number_of_duration_processes'] ?: 1)) / 1000 / 1000, 3).' seconds';

		foreach ($php_data['totals']['requests_by_uri'] as $md5_uri => $request_data)
		{
			$php_data['totals']['requests_by_uri_string'] .= '<tr><td>'.$request_data['counter'].'</td><td>'
				.$request_data['uri'].'</td></tr>';
		}
